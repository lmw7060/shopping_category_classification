...
for i in ca:
    url = 'https://search.shopping.naver.com/search/category/10000{}'.format(i)
    resp = requests.get(url, headers=headers)
    soup = BeautifulSoup(resp.text, 'html.parser')
    title_tags = soup.select('.adProduct_link__NYTV9')
    print(title_tags)
    titles = []
    for title_tag in title_tags:
...
df_titles.to_csv('./crawling_data/naver_shopping_{}.csv'.format(datetime.datetime.now().strftime('%Y%m%d')), index=False)
#이 경로에 이런 파일명으로 넣겠다
